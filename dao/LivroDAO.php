...
class LivroDAO
{
    ...
    {
        $sql = "INSERT INTO livros (titulo, autor, genero_id, preco, editora, descricao, imagem_url, created_at, updated_at)
    ...
    {
        $sql = "UPDATE livros SET titulo = :titulo, autor = :autor, genero_id = :genero_id, preco = :preco, editora = :editora, descricao = :descricao, imagem_url = :imagem_url, updated_at = NOW() WHERE id = :id";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([
            ':id' => $id,
            ':titulo' => $titulo,
