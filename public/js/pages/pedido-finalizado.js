...
async function carregarDadosPedido(pedidoId) {
  try {
    ...
    console.log(
      "Resposta da API (buscarPedidoCompletoPorId):",
...
function processarDadosPedido(dadosPedidoApi) {
  if (!dadosPedidoApi || !dadosPedidoApi.id) {
  ...
  if (!Array.isArray(itensDoPedido)) {
    console.warn(
      "Itens do pedido não são um array ou estão ausentes. Tratando como lista vazia.",
      itensDoPedido
    );
    itensDoPedido = [];
  }
...
function preencherInformacoesPedido(pedidoInfo) {
  pedidoNumero.textContent = `#${pedidoInfo.id.toString().padStart(4, "0")}`;
  ...
  div.innerHTML = `
        ...
        <div class="order-item-details">
            ...
            <div class="order-item-info-row">
                ...
                <span>Preço unitário: ${formatarPreco(
                  item.preco_unitario
                )}</span>
            </div>
            <div class="order-item-info-row">
                <span>Tipo: ${tipoFormatado}</span>
                <span class="order-item-price">Total: ${formatarPreco(
                  item.total
                )}</span>
            </div>
        </div>
